"""Metrics, computed in-process, plus a costed judge-model metric in `judge.py`.
...
over a large sample, not in the path of every pull request — see `judge.py` for
that one and why it is a separate module rather than a sixth function here: it
needs a `LanguageModel` and a price, neither of which any function below does.
